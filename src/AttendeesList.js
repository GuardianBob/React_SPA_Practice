...
class AttendeesList extends Component {
    ...
    render() {
        ...
        const myAttendees = attendees.map(item => {
            return (
                <div className="col-8 col-sm-6 col-md-6 col-lg-6 mb-2 p-0 px-1 "
                    key={item.attendeeID}>
                    <div className="card ">
                        <div className={
                            'card-body px-3 py-2 d-flex align-items-center ' + 
                            (admin ? '': 'justify-content-center')  // This adds extra HTML if user is admin user!
                        }>
                            ...
                            {admin&& (
                                <div className="btn-group pe-2">
                                    ...
                                    <button className="btn btn-sm btn-outline-secondary " 
                                        title="Delete Attendee" onClick={e => this.deleteAttendee(e, this.props.meetingID, item.attendeeID)}>
                                            <GoTrashcan />
                                    </button>                                    
                                </div>
                            )}
